...
export function Produccion_crearModeloFicha(ObjFicha:FichaTecnicaAnimal){
  return {
    ...
    mediciones:ObjFicha.mediciones.map(Obj=>({fecha:Produccion_fechaHoraFicha(Obj.fechaMedicion),medicion:`${Obj.tipoMedicion}: ${Obj.valor} ${Obj.unidadMedida}`,observacion:Produccion_texto(Obj.observaciones)})),
...
export async function Produccion_generarFichaPdf(ObjFicha:FichaTecnicaAnimal,ObjFoto:Blob|null):Promise<void>{
  ...
  Tabla("Pesos y mediciones",["Fecha","Medición","Observación"],ObjModelo.mediciones.map(Obj=>[Obj.fecha,Obj.medicion,Obj.observacion]));
