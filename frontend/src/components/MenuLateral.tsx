...
import { ArrPermisosConcentrados } from "./alimentacion/ConcentradosCompartidos";
...
export function MenuLateral({ BoolAbierto, Autenticacion_cerrarMenu }: PropiedadesMenuLateral) {
  ...
  const BoolConsultarAlimentacion = Autenticacion_tienePermiso("ALIMENTACION_CONSULTAR");
  const BoolMostrarAlimentacion = BoolConsultarAlimentacion || ArrPermisosConcentrados.some(Autenticacion_tienePermiso);
    ...
    <>
      ...
      <aside className={`menu-lateral ${BoolAbierto ? "menu-lateral--abierto" : ""}`} aria-label="Navegación principal">
        ...
        <nav className="menu-navegacion">
          ...
          {BoolMostrarAlimentacion && <NavLink to={BoolConsultarAlimentacion ? "/alimentacion" : "/alimentacion/concentrados"} className={({ isActive: BoolActivo }) => `menu-opcion ${BoolActivo ? "menu-opcion--activa" : ""}`} onClick={Autenticacion_cerrarMenu}><span aria-hidden="true">A</span> Alimentación</NavLink>}
