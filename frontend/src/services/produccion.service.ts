...
export const Produccion_obtenerFichaTecnica=(IntId:number)=>Api_solicitar<T.RespuestaDato<T.FichaTecnicaAnimal>>(`/api/produccion/animales/${IntId}/ficha-tecnica`);
export async function Produccion_obtenerFoto(IntId:number):Promise<Blob|null>{const ObjRespuesta=await fetch(Api_resolverUrl(`/api/produccion/animales/${IntId}/foto`),{credentials:"include"});if(ObjRespuesta.status===404)return null;if(ObjRespuesta.status===503)throw new ErrorApi(503,"ALMACENAMIENTO_NO_DISPONIBLE","Fotografía no disponible");if(!ObjRespuesta.ok)throw new ErrorApi(ObjRespuesta.status,"FOTO_NO_DISPONIBLE","No fue posible cargar la fotografía.");return ObjRespuesta.blob();}
export async function Produccion_reemplazarFoto(IntId:number,ObjArchivo:File):Promise<void>{const ObjFormulario=new FormData();ObjFormulario.set("foto",ObjArchivo);const ObjRespuesta=await fetch(Api_resolverUrl(`/api/produccion/animales/${IntId}/foto`),{method:"POST",credentials:"include",body:ObjFormulario});if(ObjRespuesta.ok)return;let StrMensaje="No fue posible guardar la fotografía.";try{const ObjContenido=await ObjRespuesta.json() as {error?:{mensaje?:string}};if(ObjContenido.error?.mensaje)StrMensaje=ObjContenido.error.mensaje;}catch{}throw new ErrorApi(ObjRespuesta.status,"FOTO_NO_GUARDADA",StrMensaje);}
...
export type DatosMedicionProduccion={animalId:number;observaciones?:string|null}&({metodoObtencion:"BASCULA";pesoKg:T.DecimalProduccion}|{metodoObtencion:"ESTIMACION_SCHAEFFER";perimetroToracicoCm:T.DecimalProduccion;longitudCorporalCm:T.DecimalProduccion});
export const Produccion_registrarMedicion=(ObjDatos:DatosMedicionProduccion)=>Api_solicitar<T.RespuestaDato<T.MedicionProduccion>>("/api/produccion/mediciones",{method:"POST",ObjCuerpo:ObjDatos});
