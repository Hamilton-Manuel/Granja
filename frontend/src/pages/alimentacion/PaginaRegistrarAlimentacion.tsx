...
import {
  Alimentacion_consultarDisponibilidad,
...
export function PaginaRegistrarAlimentacion() {
  ...
  const IntConsulta = useRef(0);
  const [ObjDisponibilidad, establecerDisponibilidad] = useState<DisponibilidadAlimentacion | null>(null);
  const [StrErrorDisponibilidad, establecerErrorDisponibilidad] = useState<string | null>(null);
  ...
  useEffect(() => {
    const IntSolicitud=++IntConsulta.current;
    establecerDisponibilidad(null);
    establecerErrorDisponibilidad(null);
    if(IntFormulaId===null || !StrFecha) return;
    void Alimentacion_consultarDisponibilidad({formulaId:IntFormulaId,fechaEfectiva:Alimentacion_fechaBackend(StrFecha)})
      .then(Obj=>{if(IntSolicitud===IntConsulta.current) establecerDisponibilidad(Obj);})
      .catch(Obj=>{if(IntSolicitud===IntConsulta.current) establecerErrorDisponibilidad(Alimentacion_mensajeError(Obj));});
    return ()=>{IntConsulta.current++;};
  },[IntFormulaId,StrFecha,IntRevision]);
  ...
  async function Alimentacion_aplicarFormula(IntId: number | null) {
    ...
    if (IntId === null) {
      // Conservar ingredientes, pero exigir la fuente explícita del consumo directo.
      establecerProcesando(true);
      try {
        const ArrNuevas=await Promise.all(ArrLineas.map(async Obj=>({...Obj,ObjAlmacen:null,ObjLote:null,ArrExistencias:Obj.ObjProducto?await Alimentacion_buscarExistencias(Obj.ObjProducto.productoId):[]})));
        establecerLineas(ArrNuevas);
      } catch(ObjError){establecerError(Alimentacion_mensajeError(ObjError));}
    ...
    establecerLineas(ObjFormula.detalles.filter(Obj=>Obj.activo).map(ObjDetalle=>({
      ...Alimentacion_nuevaLinea(IntSiguienteClave.current++),ObjProducto:ObjDetalle.producto,StrCantidad:ObjDetalle.cantidad,
    })));
  }

  async function Alimentacion_revisar() {
    const StrValidacion=Alimentacion_validar();
    if(StrValidacion){establecerError(StrValidacion);return;}
    establecerError(null);
    establecerProcesando(true);
    try {
      if(IntFormulaId!==null) {
        // La revisión reemplaza cualquier consulta previa que siga en vuelo.
        IntConsulta.current++;
        const Obj=await Alimentacion_consultarDisponibilidad({formulaId:IntFormulaId,fechaEfectiva:Alimentacion_fechaBackend(StrFecha)});
        establecerDisponibilidad(Obj);
        establecerErrorDisponibilidad(null);
        if(!Obj.disponible)return;
      }
      establecerConfirmar(true);
    } catch(ObjError){establecerError(Alimentacion_mensajeError(ObjError));}
    finally{establecerProcesando(false);}
  }

  async function Alimentacion_recargarFormulas() {
    establecerProcesando(true);
    try {
      const ObjRespuesta=await Alimentacion_listarFormulas();
      establecerFormulas(ObjRespuesta.datos.filter(Obj=>Obj.activo));
      const ObjFormula=ObjRespuesta.datos.find(Obj=>Obj.formulaId===IntFormulaId&&Obj.activo);
      if(ObjFormula) establecerLineas(ObjFormula.detalles.filter(Obj=>Obj.activo).map(ObjDetalle=>({...Alimentacion_nuevaLinea(IntSiguienteClave.current++),ObjProducto:ObjDetalle.producto,StrCantidad:ObjDetalle.cantidad})));
      else {
        establecerFormulaId(null);
        establecerLineas([Alimentacion_nuevaLinea(IntSiguienteClave.current++)]);
        establecerError("La fórmula seleccionada ya no está disponible. Seleccione otra fórmula.");
        return;
      }
      establecerError(null);
      establecerRevision(Int=>Int+1);
    }catch(ObjError){establecerError(Alimentacion_mensajeError(ObjError));}
    finally{establecerProcesando(false);}
  }
  ...
  function Alimentacion_validar(): string | null {
    ...
    for (const ObjLinea of ArrLineas) {
      if(IntFormulaId!==null) {
        if(!ObjLinea.ObjProducto) return "Recargue las fórmulas para revisar sus ingredientes.";
        continue;
      }
  ...
  async function Alimentacion_confirmarRegistro() {
    ...
    try {
      const ObjRespuesta = await Alimentacion_registrar({
        ...
        ...(IntFormulaId!==null ? {formulaId:IntFormulaId,detalles:ArrLineas.map(Obj=>({productoId:Obj.ObjProducto!.productoId,cantidad:Obj.StrCantidad}))} : {detalles: ArrLineas.map((ObjLinea) => ({
  ...
  return <div className="alimentacion-contenido">
    ...
    {StrError && <p className="mensaje-error" role="alert" style={{whiteSpace:"pre-line"}}>{StrError}</p>}
    {StrExito && <p className="mensaje-exito" role="status">{StrExito}</p>}
    <fieldset disabled={BoolProcesando} style={{border:0,margin:0,padding:0,minWidth:0}}>
    <section className="alimentacion-formulario alimentacion-registro">
      ...
      {IntFormulaId!==null && <div className="alimentacion-disponibilidad">
        <p>Los productos y cantidades corresponden a la fórmula. El inventario se asignará automáticamente al guardar.</p>
        {(ObjDisponibilidad?.mensaje || StrErrorDisponibilidad) && <p role="alert" className="mensaje-error" style={{whiteSpace:"pre-line"}}>{ObjDisponibilidad?.mensaje ?? StrErrorDisponibilidad}</p>}
        {ObjDisponibilidad?.disponible && <p role="status">Inventario disponible. Se comprobará nuevamente al guardar.</p>}
        <button type="button" onClick={()=>establecerRevision(Int=>Int+1)}>Comprobar disponibilidad</button>
        <button type="button" onClick={()=>void Alimentacion_recargarFormulas()}>Recargar fórmulas</button>
      </div>}
      <fieldset><legend>{IntFormulaId!==null?"Ingredientes de la fórmula":"Alimentos y fuentes físicas"}</legend>{ArrLineas.map((ObjLinea, IntIndice) => <article className={`alimentacion-linea-registro${IntFormulaId!==null?" alimentacion-ingrediente-fijo":""}`} key={ObjLinea.IntClave}>
        {IntFormulaId!==null ? <p><strong>{ObjLinea.ObjProducto?.nombre}</strong></p> : <Autocomplete StrEtiqueta={`Producto ${IntIndice + 1}`} StrPlaceholder="Buscar por código o nombre..." ObjSeleccion={ObjLinea.ObjProducto} Autocomplete_buscar={async (StrBusqueda) => ArrProductos.filter((Obj) => `${Obj.codigo} ${Obj.nombre}`.toLocaleLowerCase().includes(StrBusqueda.toLocaleLowerCase()))} Autocomplete_clave={(Obj) => Obj.productoId} Autocomplete_etiqueta={(Obj) => `${Obj.codigo} — ${Obj.nombre} — ${Obj.unidadMedida}`} Autocomplete_seleccionar={(Obj) => void Alimentacion_seleccionarProducto(ObjLinea.IntClave, Obj)} />}
        {IntFormulaId===null && ObjLinea.ObjProducto && <Autocomplete StrEtiqueta="Almacén" StrPlaceholder="Buscar por código o nombre..." ObjSeleccion={ObjLinea.ObjAlmacen} Autocomplete_buscar={async (StrBusqueda) => { const ArrAlmacenes = await Alimentacion_buscarAlmacenes(StrBusqueda); const ArrIds = new Set(ObjLinea.ArrExistencias.map((Obj) => Obj.inventarioId)); return ArrAlmacenes.filter((Obj) => ArrIds.has(Obj.inventarioId)); }} Autocomplete_clave={(Obj) => Obj.inventarioId} Autocomplete_etiqueta={(Obj) => `${Obj.codigo} — ${Obj.nombre}`} Autocomplete_seleccionar={(Obj) => Alimentacion_actualizarLinea(ObjLinea.IntClave, { ObjAlmacen: Obj, ObjLote: null })} />}
        {IntFormulaId===null && ObjLinea.ObjProducto && ObjLinea.ObjAlmacen && (StrFecha ? <Autocomplete StrEtiqueta="Lote de Inventario" StrPlaceholder="Buscar lote..." ObjSeleccion={ObjLinea.ObjLote} Autocomplete_buscar={async (StrBusqueda) => (await Alimentacion_buscarLotesInventario(ObjLinea.ObjProducto!.productoId, ObjLinea.ObjAlmacen!.inventarioId, Alimentacion_fechaBackend(StrFecha))).filter((Obj) => Obj.codigoLote.toLocaleLowerCase().includes(StrBusqueda.toLocaleLowerCase()))} Autocomplete_clave={(Obj) => Obj.loteInventarioId} Autocomplete_etiqueta={(Obj) => `${Obj.codigoLote} — Disponible: ${Obj.cantidadDisponible} ${ObjLinea.ObjProducto!.unidadMedida} — ${Obj.fechaVencimiento ? `Vence: ${Obj.fechaVencimiento}` : "Sin vencimiento"}`} Autocomplete_seleccionar={(Obj) => Alimentacion_actualizarLinea(ObjLinea.IntClave, { ObjLote: Obj })} /> : <p className="alimentacion-aviso">Indique la fecha efectiva para consultar lotes utilizables.</p>)}
        {ObjLinea.ObjProducto && <label>Cantidad ({ObjLinea.ObjProducto.unidadMedida})<input inputMode="decimal" readOnly={IntFormulaId!==null} required value={ObjLinea.StrCantidad} onChange={(E) => Alimentacion_actualizarLinea(ObjLinea.IntClave, { StrCantidad: E.target.value })} /></label>}
        {IntFormulaId===null && <button type="button" className="boton-secundario" disabled={ArrLineas.length === 1} onClick={() => establecerLineas((Arr) => Arr.filter((Obj) => Obj.IntClave !== ObjLinea.IntClave))}>Quitar alimento</button>}
      </article>)}{IntFormulaId===null && <button type="button" className="boton-secundario" onClick={() => establecerLineas((Arr) => [...Arr, Alimentacion_nuevaLinea(IntSiguienteClave.current++)])}>Agregar alimento</button>}</fieldset>
