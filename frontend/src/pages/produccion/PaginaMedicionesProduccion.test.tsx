...
import { fireEvent, render as Produccion_renderBase, screen, waitFor } from "@testing-library/react";
...
let BoolPuedeConsultar = true;
vi.mock("../../hooks/useSesion",()=>({useSesion:()=>({Autenticacion_tienePermiso:(Str: string)=>Str !== "PRODUCCION_CONSULTAR" || BoolPuedeConsultar})}));
...
describe("mediciones de peso",()=>{
  beforeEach(()=>{vi.clearAllMocks();ArrMediciones=[];BoolPuedeConsultar=true;});
  it("ofrece acceso al análisis únicamente con permiso de consulta",()=>{
    const ObjVista = render(<PaginaMedicionesProduccion/>);
    expect(screen.getByRole("link",{name:"Analizar ganancia de peso"})).toHaveAttribute("href","/produccion/mediciones/ganancia");
    BoolPuedeConsultar=false;
    ObjVista.rerender(<MemoryRouter><PaginaMedicionesProduccion/></MemoryRouter>);
    expect(screen.queryByRole("link",{name:"Analizar ganancia de peso"})).not.toBeInTheDocument();
  });
...
function render(ObjElemento: ReactElement) { return Produccion_renderBase(<MemoryRouter>{ObjElemento}</MemoryRouter>); }
