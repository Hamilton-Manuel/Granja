...
export const ObjConsultaProductos = ObjConsultaInventario.extend({ categoriaId: ObjId.optional(), manejaLotes: z.coerce.boolean().optional() }).strict();
...
export const ObjEditarLote = z.object({ fechaFabricacion: z.string().date().nullable().optional(), fechaVencimiento: z.string().date().nullable().optional(), observaciones: ObjTextoOpcional(1000) }).strict().refine((Obj) => Object.keys(Obj).length > 0);
...
export const ObjCrearEntrada = ObjMovimientoBase.extend({ subtipo: z.enum(["COMPRA", "INVENTARIO_INICIAL"]) }).strict();
...
export const ObjCrearAjuste = ObjMovimientoBase.omit({ costoUnitario: true, proveedorId: true, codigoLote: true, fechaFabricacion: true, fechaVencimiento: true }).extend({ cantidad: z.union([z.string(), z.number()]).transform(String).refine((StrValor) => /^-?\d+(\.\d{1,4})?$/.test(StrValor) && Number(StrValor) !== 0), subtipo: z.literal("CONTEO_FISICO") }).strict();
